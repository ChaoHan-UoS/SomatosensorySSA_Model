...
if L
    ...
    xlim([0 n_stim_plot*(duration+ISI)])
    pos = get(s3,'position');
    set(s3,'position',[0.0602,0.3715,0.8448,pos(4)+(1/3)*pos(4)],'FontSize',AXES_FONTSIZE,'XTickLabel',[]);
    ...
    xlim([0 n_stim_plot*(duration+ISI)])
    ylim([0.4 1])
    legend('Std Col', 'Rec Col', 'Dev Col')
    pos = get(s4,'position');
    set(s4,'position',[0.0602,pos(2),0.8448,pos(4)+(1/3)*pos(4)],'YTick', [0.4:0.2:1],'FontSize',AXES_FONTSIZE)
    
   
    figure(2)  
    s1 = subplot(6,1,1)
    plot(xaxis, reshape(E_act_overall(Rec_Column-Freq_Diff/2,1,onset:offset),[1,length(xaxis)]), 'Color',[0.66 0 0.94],'LineWidth',LineWidth) 
    hold on;
    plot(xaxis, reshape(E_act_overall(Rec_Column-Freq_Diff/2,2,onset:offset),[1,length(xaxis)]), 'Color', [1 0.56 0],'LineWidth',LineWidth) 
    hold on;
    xlim([0 n_stim_plot*(duration+ISI)])
    ylim([0 100])
    ylabel('E^{std}')
    ...
    s1 = subplot(6,1,2)
    plot(xaxis, reshape(x_act_overall(Rec_Column-Freq_Diff/2,1,onset:offset),[1,length(xaxis)]), 'Color', [0.66 0 0.94],'LineWidth',LineWidth) 
    hold on;
    plot(xaxis, reshape(x_act_overall(Rec_Column-Freq_Diff/2,2,onset:offset),[1,length(xaxis)]),'Color', [1 0.56 0],'LineWidth',LineWidth) 
    hold on;
    xlim([0 n_stim_plot*(duration+ISI)])
    ylabel('x^{std}')
    pos = get(s1,'position');
    ...
    ylabel('E^{rec}')
    pos = get(s2,'position');
    set(s2,'position',[0.0602    pos(2)    0.8448    pos(4)],'FontSize',AXES_FONTSIZE,'XTickLabel',[])
    ...
    legend('inh','exc')
    pos = get(s3,'position');
    set(s3,'position',[0.0602    pos(2)    0.8448    pos(4)],'FontSize',AXES_FONTSIZE)
